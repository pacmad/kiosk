...
@section('main')
    @if($children->count())
        <article class="article">
            <h2 class="headline headline--important">
                <strong class="strong">Kinder</strong>
            </h2>
            <div class="overview">
                <table class="overview__table">
                    ...
                    <tbody>
                        @foreach($children as $child)
                            <tr class="overview__row">
                                <td class="overview__cell">{{ $child->name }}</td>
                                <td class="overview__cell">{{number_format($child->amount, 2, ',', '.')}} €</td>
                                <td class="overview__cell">
                                    <a href="{{action('GodparenthoodController@show', ['godparenthood' => $child->godparenthood])}}">
                                        {{ $child->godparenthood->title }}
                                    </a>
                                </td>
                                <td class="overview__cell overview__cell--fit-to-content">
                                    <a class="button button--primary button--without-text" href="{{action('ChildController@show', ['child' => $child])}}">
    ...
    @endif
    @if($godparenthoods->count())
        <article class="article">
            <h2 class="headline headline--important">
                <strong class="strong">Patenschaften</strong>
            </h2>
            <div class="overview">
                <table class="overview__table">
                    <thead class="overview__header">
                        <tr class="overview__row">
                            <th class="overview__cell">Titel</th>
                            <th class="overview__cell">Kinder</th>
                            <th class="overview__cell"></th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($godparenthoods as $godparenthood)
                            <tr class="overview__row">
                                <td class="overview__cell">{{ $godparenthood->title }}</td>
                                <td class="overview__cell">{{ $godparenthood->children->count() }}</td>
                                <td class="overview__cell overview__cell--fit-to-content">
                                    <a class="button button--primary button--without-text" href="{{action('GodparenthoodController@show', ['godparenthood' => $godparenthood])}}">
                                        <svg class="button__icon" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="20px" height="20px" viewBox="0 0 511.626 511.626" style="enable-background:new 0 0 511.626 511.626;" xml:space="preserve">
                                            <g>
                                                <path fill="#000" d="M505.918,236.117c-26.651-43.587-62.485-78.609-107.497-105.065c-45.015-26.457-92.549-39.687-142.608-39.687
                                                    c-50.059,0-97.595,13.225-142.61,39.687C68.187,157.508,32.355,192.53,5.708,236.117C1.903,242.778,0,249.345,0,255.818
